...
public class RestServer implements Closeable, Runnable, IServer {
	...
	public void run() {
		...
		resourceConfig.getProperties().put("com.sun.jersey.spi.container.ContainerRequestFilters",
				"streamServer.AuthorizationFilter");
	...
	@Override
	public Response contentList(Request request) {
		try {
			return Response.status(Status.OK)
					.entity(new WrappedList())
					.build();
		} catch (Exception ex) {
			return Response.status(Status.INTERNAL_SERVER_ERROR).entity(ex.getMessage()).build();
		}
	...
	@Override
	public Response getStream(Request request, String netStr) {
		UserInfo srcUser = getUser(request);
		if (srcUser == null){
			return Response.status(Status.UNAUTHORIZED).entity("Failed to recognise the user credentials.").build();
		}else if(new WrappedList().items.isEmpty()) {
			return Response.status(Status.NO_CONTENT).build();
		} else {
		Thread t = new Thread(){ 
	        public void run(){
	        	try {
					StreamHandler.startStream(netStr);
				} catch (Exception e) {
					e.printStackTrace();
				}
	        }
